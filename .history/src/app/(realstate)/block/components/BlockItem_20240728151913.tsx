...
import { CreateBlock, updateBlock } from "../services/block";
...
export default function BlockItem({ block }: BlockItemProps) {
  ...
  const res = block.id
  const updateBlockForm = async ({ block, description }: CreateBlock,actions:FormikHelpers<Block>) => {
    const res = await updateBlock({block,description},res);
  };
  return (
    <>
